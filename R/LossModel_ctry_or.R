...
LossModel_ctry <- function(Data,timeSeriesDataToBeImputed,ctry_modelvar,HierarchicalCluster,keys_lower){
  ...
  modelversion <- 1.12 #packageVersion("faoswsLoss")
  ...
  for (vii in 1:length(na.omit(unique(Data$geographicaream49)))){
     ...
     # CPCs <- unique(data_byctry$measureditemcpc)
      ...
      for(viii in unique(data_byctry$measureditemcpc)){
        ...
        if((dim(data_byctry[measureditemcpc == viii,"loss_per_clean",with=F])[1]>1) &
          ...
          timeSeriesDataToBeImputed[is.na(protected) & loss_per_cleana >0,flagcombination := 'I;i',]
      ...
      if(dim(datamod_ctry)[1] < 4){
        print("move to the global model ")
        next
      }
        ...
        for(ir in 1:length(na.omit(ImportVar2))){
          for( j in unique(datapred$geographicaream49)){
            ...
            if(is.na(sum(datapred[geographicaream49 == j,ImportVar2[ir],with=F] ))){
              ...
              datapred[geographicaream49 == j,ImportVar2[ir]] <- with(datapred[geographicaream49 %in% j,], x_impute(datapred[[ImportVar2[ir]]], mean))
              datapred[,ImportVar2[ir]] <- na.approx(datapred[,ImportVar2[ir],with=F], na.rm = T)
            }
            datamod_ctry[geographicaream49 == j,ImportVar2[ir]] <- with(datamod_ctry[geographicaream49  %in% j,], x_impute(datamod_ctry[[ImportVar2[ir]]], mean))
      ...
      modrun =1
      modelCI <- tidy(mod2,conf.int = TRUE)
     #  ### CI 
     #  years <-datamod_ctry$timepointyears
     #  measureditemcpc_ <- as.character(datamod_ctry$measureditemcpc)
     #  
     #  pred_interval <- prediction(mod2_rand, data = find_data(mod2_rand, parent.frame()),
     #                               at = NULL, calculate_se = TRUE)
     #  conf_interval <- predict(mod2, data =  find_data(model, parent.frame()) , 
     #          interval = "confidence", level = 0.95)
     #  
     #  
     #  
     # plot( losstransf ~ timepointyears , data = datamod_ctry, type="n")
     #  
     #  DescTools::DrawBand(y = pred_interval[, 2:3],
     #                      x = rangex[,1], col = rgb(0.9, 0.9, 0.9, 0.5))
     #  DescTools::DrawBand(y = conf_interval[, 2:3],
     #                      x = rangex[,1], col = rgb(0.8, 0.8, 0.8, 0.5))
     #  points(Y2016 ~ Y2012, data = election,
     #         pch = 16, col = rgb(0, 0, 0, 0.5))
     #  abline(lin.model, col = "red")
     #  
     #  legend("topleft", legend=c("Prediction interval", "Confidence Interval"),
     #         pch=15, col=c("grey90", "grey80"))
      ...
      modelag2 <- unique(unlist(datapred[(value_measuredelement_5126< 0) & (value_measuredelement_5126< 3*sd(data_byctry$loss_per_clean, na.rm = T)),"measureditemcpc",with=F]))
      datapred[measureditemcpc %in% modelag2,protected := FALSE]

      if(length(modelag)>0){
        datapred[loss_per_clean > median(data_byctry$loss_per_clean, na.rm = T) + 3*sd(data_byctry$loss_per_clean, na.rm = T),flagcombination := 'I;m',]
        datapred[loss_per_clean > median(data_byctry$loss_per_clean, na.rm = T) + 3*sd(data_byctry$loss_per_clean, na.rm = T),"loss_per_clean" := median(data_byctry$loss_per_clean, na.rm = T),]
        datapred[loss_per_clean <.01 ,flagcombination := 'I;m',]
        datapred[loss_per_clean <.01,"loss_per_clean" :=  median(data_byctry$loss_per_clean, na.rm = T),]
        datapred[is.na(loss_per_clean ),flagcombination := 'I;m',]
        datapred[is.na(loss_per_clean ),"loss_per_clean":= median(data_byctry$loss_per_clean, na.rm = T),] 
      ...
      ModelRes <-list("geographicaream49_" = unique(datapred[flagcombination == 'I;ec',"geographicaream49",with=F]), "measureditemcpc_" = unique(datapred[flagcombination == 'I;ec',"measureditemcpc",with=F]),
                      "model_CI"= as.data.table(modelCI), "model_T"= "Extended")
      ModelRes2 <-list("geographicaream49_" = unique(datapred[flagcombination == 'I;es',"geographicaream49",with=F]), "measureditemcpc_" = unique(datapred[flagcombination == 'I;es',"measureditemcpc",with=F]),
                       "model_CI"= as.data.table(modelCI2), "model_T"= "Simplified")
      
      ModelRes3 <-list("geographicaream49_" = unique(datapred[flagcombination == 'I;m' ,"geographicaream49",with=F]), "measureditemcpc_" = unique(datapred[flagcombination == 'I;m' & protected == T,"measureditemcpc",with=F]),
                       "model_CI"= NA, "model_T"= "Averaged")
      ...
      timeSeriesDataToBeImputed[is.na(protected) & loss_per_cleana >0,flagcombination := flagcombinationa,]
      ...
      modelRes_Comod <- list(ModelRes,ModelRes2, ModelRes3)
    ...
    modelRes_Ctry <- list(modelRes_Ctry,modelRes_Comod)
  }
  modelRes_Ctry2 <- list(modelRes_Ctry,modelRes_Ctry2)
  end <- Sys.time()
  print(end - start)
  ...
  capture.output(modelRes_Ctry, file = "ctry_ModelResults.txt")
