...
SaveDataNew <- function(domain, dataset, data, metadata, normalized = TRUE, waitMode = "wait", waitTimeout = 600, chunkSize = 50000) {
  ...
  if (waitMode != "synch" & (nRowData > chunkSize | nRowData > chunkSize)) {
    uuid <- SaveDataNew.generateUuid()
    if (!missing(data)) {
      json <- NULL
      if (normalized) {
        json <- SaveDataNew.buildNormalizedDataDefJSON(data)
      } else {
        json <- SaveDataNew.buildDenormalizedDataDefJSON(data)
      }
      PutRestCall(paste0(baseUrl, "stream/", swsContext.executionId, "/", 
          uuid, "/data/def?token=", swsContext.token), 
        json)
      dataChunks <- SaveDataNew.splitIntoChunkTables(data, chunkSize)
      dataChunksCnt <- length(dataChunks)
      for (i in 1 : dataChunksCnt) {
        if (normalized) {
          json <- SaveDataNew.buildNormalizedDataContentJSON(dataChunks[[i]])
        } else {
          json <- SaveDataNew.buildDenormalizedDataContentJSON(dataChunks[[i]])
        }
        PutRestCall(paste0(baseUrl, "stream/", swsContext.executionId, "/", 
    ...
    if (!missing(metadata)) {
      PutRestCall(
        paste0(baseUrl, "stream/", swsContext.executionId, "/", 
          ...
          SaveDataNew.buildMetadataDefJSON(metadata))
      metaChunks = SaveDataNew.splitIntoChunkTables(metadata, chunkSize)
      metaChunksCnt <- length(metaChunks)
      for (i in 1 : metaChunksCnt) {
        PutRestCall(paste0(baseUrl, "stream/", swsContext.executionId, "/", 
            uuid, "/meta/chunk/", (i - 1), "?token=", swsContext.token),
          SaveDataNew.buildMetadataContentJSON(metaChunks[[i]]))
      }
    }
    out <- PutRestCall(
    ...
    out <- PutRestCall(
      ...
      SaveDataNew.buildUniqueJSON(data, metadata, normalized)
    )
    uuid <- out[['message']]
  }
...
SaveDataNew.generateUuid <- function() {
...
SaveDataNew.splitIntoChunkTables <- function(tbl, chunkSize) {
...
SaveDataNew.buildUniqueJSON <- function(data, metadata, normalized) {
  
  json <- list()
  
  if(!missing(data)) {
    if(normalized) {
      json[["data"]] <- SaveDataNew.buildNormalizedDataDefJSON(data)
      json[["data"]][["data"]] <- SaveDataNew.buildNormalizedDataContentJSON(data)
    } else {
      #json[["denormalizedData"]] <- SaveDataNew.buildDenormalizedDataJSON(data)
      json[["data"]] <- SaveDataNew.buildDenormalizedDataDefJSON(data)
      json[["data"]][["data"]] <- SaveDataNew.buildDenormalizedDataContentJSON(data)
    }
  }
  
  if(!missing(metadata)) {
    json[["metadata"]] <- SaveDataNew.buildMetadataDefJSON(metadata)
    jsonMetacont <- SaveDataNew.buildMetadataContentJSON(metadata)
...
SaveDataNew.buildNormalizedDataDefJSON <- function(data) {
...
SaveDataNew.buildNormalizedDataContentJSON <- function(data) {
...
SaveDataNew.buildDenormalizedDataDefJSON <- function(data) {
...
SaveDataNew.buildDenormalizedDataContentJSON <- function(data) {
...
SaveDataNew.buildMetadataDefJSON <- function(metadata) {
...
SaveDataNew.buildMetadataContentJSON <- function(metadata) {
