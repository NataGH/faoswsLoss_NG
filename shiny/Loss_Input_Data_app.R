...
FAOCrops <- FAOCrops[measureditemcpc %in% LossFactorRaw$measureditemcpc,]
...
uniqeCpC <- unique(LossFactorRaw[,"measureditemcpc",with=F])
...
LossFactorRaw[Stage =="SWS_Total","Stage"] = "Official/Semi-Official - Whole chain Estimate"

LossFactorRaw2 <- merge(LossFactorRaw,gfli_basket, by=c("measureditemcpc"))
r <-unique(LossFactorRaw2[is.na(gfli_basket),c("measureditemcpc","Crop"),with=F])
...
agglist <-c(names(CountryGroup)[seq(2,length(names(CountryGroup)), by=2)][-8],"FAO Operational Region")
agglist <- agglist[!agglist %in% c("ISO2code")]
...
Losses <- merge(Losses,CountryGroup, by=c("geographicaream49"))
Losses <- merge(Losses,gfli_basket, by=c("measureditemcpc"))
Losses
