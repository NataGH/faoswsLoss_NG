...
server <- function(input, output, session) {
  ...
  plotly_Input = function() {
    ...
    agg_a <- "geographicaream49"
    agg_b <- "measureditemcpc"
    test <- Losses
    
    ## decrease dimensions by one
    opt_z1 <- unique(test[[agg_a]])[2]
    opt_y <-  unique(test[(test[[agg_a]]  %in% opt_z1) & (value_measuredelement_5126 >0),agg_b,with=F])
    opt_y1 <-  unlist(opt_y[1])
    dataB<-  test[(test[[agg_a]]  %in% opt_z1) & (test[[agg_b]] %in% opt_y1),]
    ...
    layout(
        ...
        margin = list(l =80, r = 50, t = 60, b = 100),
