...
server <- function(input, output, session) {
  ...
  DataOutput <- function()({
    dataR()[ ,c("geographicaream49","isocode","Country","Region","measureditemcpc","Crop","Year","Average Quantity Loss (%)","Range of Quantity Loss (%)",
                "Loss (quantity, tons)","Loss (Qualitative, tons)","Loss (Monetary, LCU)",              
                "Activity","Food Value Chain Stage","period of storage","treatment","Causes of loss","Sample Size",                 
                "Sampling Units","Method of Data Collection","Data Collection Tag","Reference","Url"),with=FALSE]
  })
